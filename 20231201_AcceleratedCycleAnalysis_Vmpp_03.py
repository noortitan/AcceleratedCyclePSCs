...
def calculate_median_slope_arrhenius(df_fitting):
    ...
    # parameter_list = ['cycle_PCEmppArea_norm_slope','cycle_PCEmppArea_norm_intercept',
    #                   'cycle_VmppArea_norm_slope','cycle_VmppArea_norm_intercept',
    #                   'time_PCEmppArea_norm_slope','time_PCEmppArea_norm_intercept',
    #                   'time_VmppArea_norm_slope','time_VmppArea_norm_intercept',
    #                   'cycle_PCEmppArea_slope','cycle_PCEmppArea_intercept',
    ...
    # ylim_dict = dict([
    #     ('cycle_PCEmppArea_norm_slope',[-0.0009, 0.00045]),
    #     ('cycle_PCEmppArea_slope',[-0.035, 0.07]),
    #     ('time_PCEmppArea_norm_slope',[-0.00035, 0.00025]),
...
#     (df_calculated_area_45C['PCEmppArea_median_perHour'])[df_calculated_area_45C['cycle']==1].values[0]
    ...
    # List to stores all the values generated
    list_equivalent_45_time_log = []
    list_equivalent_45_time_log_linear = []
    list_equivalent_45_time_power = []
    list_equivalent_45_time_exp = []
...
# For the 35C
cell_params_35C = {0: [0, 'Zahra-Batch1'],
                   1: [1, 'Zahra-Batch2'], # NiOx-based
                   2: [0, 'Zahra-Batch1'], 
                   3: [1, 'Zahra-Batch2'], # NiOx-based
                   4: [2, 'Ulas'], # SAM-based
                   5: [2, 'Ulas'], # SAM-based
                   6: [2, 'Ulas'], # SAM-based 
                   7: [2, 'Ulas']} # SAM-based
...
#%% Running code: plot the boxplot of the backcalculation not norm + FITTING THE MEDIAN
...
length_base = {25:12,35:6,45:3,55:1.5}
...
for i in range(len(list_all_backcalculations_notNorm)):
    ...
    base_T_C = [25,35,45,55]
    base_T_K = [x + 273 for x in base_T_C]
    
    # Go to every base temperature
    for base_T in base_T_C:
        ...
        try:
            ...
            # Loop through combination of base_T and non_base_T
            for key_ending_base_T,non_base_T_C_specific in zip(keys_ending_with_base_T,non_base_T_C):
                ...
                # df_interest_plot_specific=(specific_base_T_df[column_interest].to_frame()).dropna()
                ...
                # df_interest_plot = df_interest_plot.append(df_interest_plot_specific, ignore_index=True)
                df_interest_plot = pd.concat([df_interest_plot, df_interest_plot_specific], ignore_index=True)
                # sns.boxplot(data = df_interest_plot, x= 'T_target',y=column_interest,ax=ax)
                
                # sns.boxplot(x=non_base_T_C_specific,y=specific_base_T_df[column_interest])
                # specific_base_T_df
                # list_column_interest = 'eq_45C_wrt_35C_time_linear_log'
                
            # Now trying to fit the data
            ...
            x_fit_short = np.array((25,35,45,55))
